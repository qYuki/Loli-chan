...
class Owner:
    ...
    async def load(self, *, module: str):
        ...
        try:
            ...
            await self.bot.say("Modul ni bil dobljen pizda.")
        except CogLoadError as e:
            log.exception(e)
            traceback.print_exc()
            await self.bot.say("Bil je problem nalozit modul. Preveri"
                               " konzolo za informacije.\n"
                               "\nError: `{}`".format(e.args[0]))
        except Exception as e:
            log.exception(e)
            traceback.print_exc()
            await self.bot.say('Modul je bil najden in mogoce nalozen ampak '
                               'nekaj je slo narobe. Preveri konzolo '
                               'ali zapiske.\n\n'
                               'Error: `{}`'.format(e.args[0]))
        else:
            set_cog(module, True)
            await self.disable_commands()
            await self.bot.say("Modul vklopljen.")
    ...
    async def _set(self, ctx):
        """Changes Vodopivec's global settings."""
    ...
    async def owner(self, ctx):
        ...
        if self.bot.settings.no_prompt is True:
            await self.bot.say("Console interaction is disabled. Start Vodopivec "
    ...
    async def prefix(self, ctx, *prefixes):
        """Sets Vodopivec's global prefixes
        ...
        log.debug("Stimam globalen prefix na:\n\t{}"
                  "".format(self.bot.settings.prefixes))

        p = "prefixes" if len(prefixes) > 1 else "prefix"
        await self.bot.say("Globalen {} nastiman".format(p))
    ...
    async def serverprefix(self, ctx, *prefixes):
        ...
        if prefixes == ():
            ...
            await self.bot.say("Server prefixi zresetirani. Trenutni prefixi: "
                               "`{}`".format(current_p))
            return
        ...
        self.bot.settings.save_settings()
        log.debug("Stimam serverjeve {} prefixe na:\n\t{}"
                  "".format(server.id, self.bot.settings.prefixes))

        p = "Prefixes" if len(prefixes) > 1 else "Prefix"
        await self.bot.say("{} nastiman za ta server.\n"
                           "Da gres nazaj na normalne prefixe naredi"
                           " `{}set serverprefix` "
                           "".format(p, prefixes[0]))
    ...
    async def game(self, ctx, *, game=None):
        """Sets Vodopivec's playing status
    ...
    async def stream(self, ctx, streamer=None, *, stream_title=None):
        """Sets Vodopivec's streaming status
    ...
    @commands.command()
    async def info(self):
        """Shows info about Vodopivec"""
        author_repo = "Niko Leban"
        red_repo = author_repo + "my dick"
        server_url = "flaske"
        ...
        about = (
            "Js sm Vodopiwc djoporko({}) "
            "ti fuknem flasku u fris\n\n"
            "al kej druzga? "
            "poberi smeti djoporko cene jih bos z jezikam({}) "
            "Sprogramjru - Niko\n\n"
        ...
        embed.add_field(name="About Vodopiwc", value=about, inline=False)
    ...
    def _unload_cog(self, cogname, reloading=False):
        if not reloading and cogname == "cogs.owner":
            raise OwnerUnloadWithoutReloadError(
                "Neupam nalozit owner plugina djoporko!")
        try:
    ...
    def _wait_for_answer(self, author):
        print(author.name + " je si zazelu bit owner. Ce si to ti, "
              "napisi 'yes'. Drugace prtisni enter alpa ti fuknem flasku u fris.")
        print()
        print("*NE* dajat drugim owner. To ima varnostne "
              "ukore.")
        ...
        if choice == "yes":
            ...
            self.bot.settings.save_settings()
            print(author.name + " je ratu owner.")
            self.setowner_lock = False
            self.owner.hidden = True
        else:
            print("Nov owner je biw ignorjran.")
            self.setowner_lock = False
